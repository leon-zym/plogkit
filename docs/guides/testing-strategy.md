...
行为变化先更新对应 spec；架构决策变化先新增 ADR。测试按风险选择，不把完整 E2E 绑定到每次提交。

| 场景             | 验证方式                                                                                    |
| ---------------- | ------------------------------------------------------------------------------------------- |
| 日常开发         | 运行受影响的 Jest 测试；用 `pnpm check` 获取快速静态反馈                                    |
| 功能开发后       | 运行 `pnpm check` 和相关 Jest；渲染变化追加 L3，系统 UI 或单平台行为变化追加对应平台 L4     |
| 准备 commit / PR | 始终运行 `pnpm verify`；关键流程、原生配置、照片选择器、持久化或导出变化追加完整 `pnpm e2e` |
| 大范围或版本迭代 | 开发中使用定向测试，里程碑运行 `pnpm verify`；PR 前运行双端 L4、手动 CI E2E 和双端真机冒烟  |
