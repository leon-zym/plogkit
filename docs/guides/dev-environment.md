...
GitHub Actions 在 PR 中执行 Android Debug 原生集成编译；定时任务并行运行双端完整 E2E，手动任务可选择平台和 flow。测试层级和适用场景见[测试策略](testing-strategy.md)，编排决策见 [ADR 0019](../adr/0019-cross-platform-maestro-e2e.md)。
