...
#### Scenario F06-S02: 打开有效草稿

- GIVEN 一个持久化内容完整的草稿
- WHEN 用户打开该草稿
- THEN 应用进入 Editor 并展示草稿内容

#### Scenario F06-S03: 从草稿库继续终止前的草稿
...
#### Scenario F06-S06: 后台切换不丢失

- GIVEN 用户处于编辑中
- WHEN 应用进入后台超过数分钟后返回
- THEN 编辑状态完整保留

#### Scenario F06-S07: 返回草稿库保留同进程撤销记录

- GIVEN 用户正在编辑一个草稿
- WHEN 用户返回草稿库后，在同一进程内再次打开该草稿
- THEN 之前的撤销与重做记录仍然可用

#### Scenario F06-S08: 成功切换草稿后重置撤销记录
