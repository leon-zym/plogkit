...
- external snapshot 不包含持久化 `schemaVersion`。整份记录合并、schema version 2、JSON 校验与迁移、文件位置及 Expo 文件 adapter 都属于 implementation；Expo 文件 adapter 与内存文件 stand-in 构成 module 内部的 local-substitutable seam。文件恢复只判断候选能否完整读取为 JSON object，schema 与字段校验仍由 App Settings module 负责，因此损坏记录的默认值降级语义不被文件 adapter 改写。
...
- 设置更新采用 durable-first：implementation 先写入包含 schema version 2 的下一记录，成功后才替换权威快照并通知订阅者。Expo SDK 57 没有为当前跨平台 API 提供原子覆盖的契约，因此 Expo 文件 adapter 使用固定的 `current/pending/backup` 三候选完成可恢复替换：先完整准备 `pending`，再保留 `backup` 后安装 `current`，验证成功后清理 sidecar；读取发现 `current` 缺失时优先恢复 `backup`，没有可恢复旧记录时才安装完整的 `pending`。该协议不保留设置历史，也不暴露恢复选择。预期存储失败以 typed result 返回并保留旧快照；invariant 或 programming error 才抛出。
- `EditorRuntime` 创建新草稿时读取 module 的当前 metadata 默认值，不长期缓存设置快照。screen 只保留保存中、错误和菜单开关等瞬时 UI 状态。
- 当前只允许 PlogKit 主进程写设置。除上述单记录、固定候选的 adapter 内部替换外，不增加跨进程锁、revision/CAS、journal、设置版本历史、恢复 UI 或草稿 aggregate 的可恢复维护。若 Share Extension 或其他进程成为真实 writer，必须以新 ADR 重新设计跨进程 seam。

## 影响与代价

- 两个 caller 不再理解持久记录，丢失更新与乐观状态分裂集中由一个小 interface 消除。
- durable-first 使设置在存储较慢时暂时保持旧值；对应交互需要在 intent 执行期间禁用，并在失败时显示可重试错误。
- 可恢复替换最多暂存一份待安装记录和一份旧记录，并增加启动时的有限修复与清理；其状态空间固定在单个设置记录内，不扩展为草稿级恢复生命周期。
