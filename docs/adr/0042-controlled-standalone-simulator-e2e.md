...
- Android SDK 的 Platform Tools 与 Emulator 由 stable package path 安装后再精确校验，Command-line Tools 使用带版本的 package path。因此当上游 stable 更新时会 fail closed，而不是自动回退到历史二进制。这是明确的维护取舍，不应被表述为可移植的不变安装。
