...
字体文件读取不进入同步 layout interface。预期的 environment/font unavailable 返回 typed layout failure，不退回估算 geometry 或静默替换字体；module invariant、释放后继续使用等编程错误才抛异常。当前 Skia implementation 可同时服务设备预览、headless 测试与 Skia export backend，但不进入 `ExportPipeline` external seam。
