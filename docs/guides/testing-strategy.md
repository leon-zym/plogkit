...
Maestro 在 iOS Simulator 和 Android Emulator 上驱动 PlogKit development build，JS bundle 由 Metro 提供。CI 固定 2.7.0 作为可复现基线；本地要求不低于 2.7.0，runner 拒绝缺失或过旧版本，对更新版本给出偏离基线提示但允许继续，以实际全量 E2E 结果判断兼容性。runner 不自动改变开发机环境。
...
- Android 与 iOS 业务 flow 通过 Maestro `setClipboard` / `pasteText` 使用相同的 Unicode 输入，包括中文与 emoji；不得以 ASCII fallback 代替平台验收。
- `clearState` 会重置应用数据。dev menu 的自动界面由项目 config plugin 禁用，避免干扰业务元素定位。
- 本地与 CI 共用同一编排脚本。双端本地测试串行完成设备准备、安装和 warmup，再并行运行两个 Maestro 业务 suite；CI 的平台 job 在不同 runner 上并行。具体命令行为和环境要求见[开发环境](dev-environment.md)。
...
Android runner 在 `sys.boot_completed=1` 之后逐项确认 boot animation 停止、设备已 provisioned、真实 launcher 已替换临时 `FallbackHome`、UI hierarchy 可响应，且不存在 System UI ANR 对话框；安装 App 与注入照片后、warmup 前再次执行同一功能性检查。两端 runner 在构建或测试前校验 Maestro 最低版本；iOS 还会校验 Xcode 和 Simulator runtime，Xcode beta 工具链输出警告而非静默失败。
...
`app-crash`、`xctest-driver` 和 `system-ui` 失败会结合 Maestro process output 与 flow artifact 分类；iOS 自动收集 DiagnosticReports 和 simulator log，Android 自动收集 logcat 与 ANR traces。详见[开发环境](dev-environment.md)。
