...
- 状态：已接受
- 接受日期：2026-07-17
- 关联：[ADR 0003](0003-document-driven-architecture.md)、[ADR 0005](0005-text-editing-model.md)、[ADR 0011](0011-testing-strategy.md)、[ADR 0021](0021-edit-commit-module.md)、[ADR 0023](0023-export-preset-catalog-and-pipeline.md)、[F01](../specs/F01-add-text.md)
...
- Skia Paragraph 是当前基础排版几何来源，不是永远完整的最终边界。当前版本的 `visualBounds` 由实际 line metrics 并集与 Paragraph 高度产生；未来描边、阴影或其他花字效果由同一 module 在基础边界上加入 effect outsets，不允许 UI 另建效果命中公式。
...
- 上述顺序只定义当前文本块彼此之间的层叠，不是未来异构自由画布的永久全局图层 schema。自由画布可以由 compositor 统一排序图片放置元素与文本块，并按文本 ID 取得既有布局；当前不增加 `zIndex`、generic `CanvasElement` union 或图层 registry。
...
文本资源的身份解析、来源选择和加载位于 text block layout module 上游；同步 layout interface 只接收已经就绪的 environment，不拥有这些资源的身份、来源或加载 lifecycle。预期的 environment/font unavailable 返回 typed layout failure，不退回估算 geometry 或静默替换字体；module invariant、释放后继续使用等编程错误才抛异常。当前 Skia implementation 可同时服务设备预览、headless 测试与 Skia export backend，但不进入 `ExportPipeline` external seam。
...
- 测试以 `createTextLayoutSnapshot()` 的 geometry、绘制结果和资源生命周期为 surface，覆盖可见边界、触达扩展、中文换行、fallback、层叠命中与资源释放。CanvasKit golden 不替代设备验收，也不要求与设备逐像素相同。
