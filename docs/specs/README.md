...
- Spec 只描述可通过受支持的产品交互进行黑盒判定的行为；只有检查内部状态或实现过程才能确认的事实不属于 spec。失败反馈、取消、恢复或重启后的状态只要能从产品外部判定，仍属于用户可观察行为。
- “范围”只概述复杂功能的边界，不引入 Scenario 中没有的独有需求。“非目标”只保留与当前功能关系紧密、容易被误解为已支持的用户侧排除项。
- 场景采用 `#### Scenario:` 标题，每个 Scenario 至少包含一个 GIVEN、WHEN 和 THEN；AND 可选。
- 场景描述**行为**而非实现；实现约束引用对应 ADR。
- “关联”只链接理解当前 spec 所需的直接上下文；普通关联不要求双向，没有直接关系时省略该字段。
- spec 是活文档：需求变化时先改 spec 再改实现；开发时先写对应失败测试再实现。
- 整体状态只使用`草拟`、`已确认`或`已实现`，并作为功能交付状态的 canonical fact。`草拟`表示尚未形成产品承诺，`已确认`表示已经承诺但尚未全部交付，`已实现`表示全部非例外 Scenario 已交付。
